...
import { ToggleRow, sectionHeadingClass } from "./shared";

import type { PriorityLevel, Project, SessionTransport, TaskConflict } from "../../types";
...
export function ProjectTab() {
  ...
  useEffect(() => {
    setAgent(project?.default_agent ?? "");
    setModel(project?.default_model ?? "");
    setConfirmDelete(false);
  }, [project?.id, project?.default_agent, project?.default_model]);
  ...
  const handleDelete = useCallback(async () => {
    if (!activeProjectId) return;
    try {
      await invoke("remove_project", { id: activeProjectId });
      removeProjectFromStore(activeProjectId);
    } catch (e) {
  ...
  }, [activeProjectId, removeProjectFromStore]);
  ...
  const handlePickIcon = useCallback(async () => {
    ...
    try {
      ...
      if (selected) {
        clearIconCache(activeProjectId);
        handleUpdate({ iconPath: selected });
      }
    } catch {
      // User cancelled
    }
  }, [activeProjectId, handleUpdate]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── Task Storage ── */}
      <div className={panelClass}>
        <div className={sectionHeadingClass}>Task Storage</div>
        <ToggleRow
